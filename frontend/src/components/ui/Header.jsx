import { Link } from 'react-router-dom';
import { useTranslation } from 'react-i18next';
import LanguageSwitcher from './LanguageSwitcher';

export default function Header({ title }) {
  const { t } = useTranslation();

  return (
    <header>
      {/* Top nav bar */}
      <div className="bg-[#f5f5f5] relative z-20 pb-0">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          {/* Left - FAQ */}
          <Link
            to="#"
            className="bg-[#C42826] text-white text-sm font-bold px-5 py-2 rounded uppercase tracking-wide hover:bg-[#a82220] transition"
          >
            {t('header.faq')}
          </Link>

          {/* Spacer for logo */}
          <div className="w-32" />

          {/* Right - Sign up + Lang */}
          <div className="flex items-center gap-3">
            <Link
              to="/register"
              className="bg-[#C42826] text-white text-sm font-bold px-5 py-2 rounded uppercase tracking-wide hover:bg-[#a82220] transition"
            >
              {t('header.signUp')}
            </Link>
            <LanguageSwitcher />
          </div>
        </div>

        {/* Logo centered, overlapping into red banner */}
        <Link to="/register" className="absolute left-1/2 -translate-x-1/2 top-2 z-30">
          <img src="/logo_mouflon.svg" alt="Trail des Mouflons d'Or" className="h-24" />
        </Link>
      </div>

      {/* Red banner with title + shapes */}
      <div className="bg-[#C42826] relative overflow-hidden">
        <img
          src="/shape.svg"
          alt=""
          className="absolute start-0 top-1/2 -translate-y-1/2 h-full opacity-80 pointer-events-none select-none"
        />
        <img
          src="/shape.svg"
          alt=""
          className="absolute end-0 top-1/2 -translate-y-1/2 h-full opacity-80 pointer-events-none select-none -scale-x-100"
        />

        <div className="relative z-10 pt-16 pb-16 text-center">
          <h1 className="text-white text-2xl md:text-3xl font-extrabold uppercase tracking-wide">
            {title || 'PAGE TITLE HERE'}
          </h1>
        </div>
      </div>
    </header>
  );
}
